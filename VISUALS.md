...
D3 v7 visualization patterns for shine explanations. For the HTML scaffold, CSS foundation, and article layout, see `ARTICLE.md`.
...
Map the narrative goal to a figure pattern. This is a starting point, not a rulebook. When in doubt, prototype two options and test which one teaches better.

| Narrative Goal | Figure Pattern | Why |
|---|---|---|
| "Removing X changed Y by Z" | Before/after bar or slope chart | Direct comparison is clearest |
| "Optimal point exists in a space" | Contour plot + draggable marker, or parameter slider | Reader explores the tradeoff |
| "Sequence of stages" | Scroll-driven narrative or stepper | Time-ordered revelation |
| "Many items, one surprising" | Sortable table with hover highlight | Density + discoverability |
| "Trend over time" | Line chart with annotation | Direction and magnitude together |
| "Distribution or spread" | Violin, box plot, or histogram | Shape matters more than a single number |
| "Parts make a whole" | Stacked bar or treemap | Proportion is the insight |
| "Connections between things" | Node-link force layout or adjacency matrix | Topology is the story |
| "Hierarchy or nesting" | Tree, sunburst, or icicle | Depth and ancestry are visible |
| "Flow through stages" | Sankey or chord diagram | Quantity and path together |

**Decision questions:**
1. Is the reader's job to *compare* values, *explore* a space, or *follow* a sequence?
2. Would interaction reveal something prose cannot? If not, use a static figure.
3. Does the figure need 2+ linked views to tell the story? If not, keep it simple.
...
The top mistakes that waste time. Each is one sentence + one-line fix.

| Gotcha | Fix |
|---|---|
| **Transitioning from undefined.** Elements fly from (0,0) because they have no initial position. | Set `attr('cx', x(d))` in the enter callback *before* calling `.transition()`. |
| **ResizeObserver infinite loop.** Chart content changes container height, triggering another resize. | Observe a CSS-sized wrapper, not the SVG itself. Set `overflow: hidden` on the wrapper. |
| **Bubble radius with `scaleLinear`.** The eye reads area, not radius. | Use `scaleSqrt` for bubble sizes: `d3.scaleSqrt().domain([0, max]).range([0, maxRadius])`. |
| **Missing `.defined()` on line generators.** Null values draw a line to (0,0). | Add `.defined(d => d.val != null)` to every `d3.line()` generator. |
| **Brush coordinates are pixels.** After resize, brush pixel positions map to wrong data values. | Convert `event.selection` to data domain immediately on brush end, store domain values, restore after resize. |
| **Forgetting `cursor: grab` on draggables.** Users don't discover drag without visual cues. | Set `cursor: grab` on the handle, `cursor: grabbing` during drag via CSS or inline styles. |
| **Tooltip positioned with `pageX`/`pageY` for `position: fixed`.** Tooltips appear at wrong coordinates after scroll. | Use `event.clientX`/`clientY` for fixed-positioned tooltips; `pageX`/`pageY` for absolute. |
| **Using `change` instead of `input` on range sliders.** Slider updates only on mouse release. | Use `input` event for continuous live feedback during drag. |
| **Creating scales inside a per-panel loop for small multiples.** Each panel gets independent scales. | Compute shared domains from the full dataset *before* the per-panel loop. |
| **Animating too fast for comprehension.** The reader can't track cause and effect in 0.5 seconds. | Err on the side of 2–3 seconds. The reader can always speed things up; they can't slow them down. |
